...
public class ClosureRequireDependencyManager {
    ...
    private Multimap<String, Node> _modules = HashMultimap.create();
    ...
    public Collection<Node> getTransitiveDependenciesFor(String moduleId, boolean includeSelf) {
    	...
    	Collection<Node> nodes = getNodes(moduleId);
    	for(Node module:nodes){
            for (String dependencyId : module.dependencyIds) {
                ...
                dependencies.addAll(getNodes(dependencyId));
            }
    	}
        return dependencies;
    }
    ...
    public Collection<Node> getNodes(String moduleId) {
    	Collection<Node> nodes = _modules.get(moduleId); 
        for(Node module:nodes){
